...
def create_artist(fname, byear, uuid):
    """
    Create an artist.
    If a record already exists with the same fname and byear,
    the old UUID is replaced with this one.
    """
    url = db['name'] + '/load'
    response = requests.post(
...
if __name__ == '__main__':
    ...
    with open('{}/artists/artists.csv'.format(resource_dir), 'r') as inp:
        rdr = csv.reader(inp)
        next(rdr)  # Skip header
        for fn, by, uuid in rdr:
            resp = create_artist(fn.strip(),
                               by.strip(),
                               uuid.strip())
            resp = check_resp(resp, 'artist_id')
            if resp is None or resp != uuid:
                print('Error creating artist {} ({}), {}'.format(fn,
                                                                 by,
                                                                 uuid))
