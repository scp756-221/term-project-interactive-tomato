...
@pytest.fixture
def artist_address(request):
    return request.config.getoption('--artist_address')


@pytest.fixture
def artist_port(request):
    return request.config.getoption('--artist_port')
...
@pytest.fixture
def artist_url(request, music_address, music_port):
    return "http://{}:{}/api/v1/artist/".format(
        artist_address, artist_port)
