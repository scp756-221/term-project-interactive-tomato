...
ARTIST_ID=e3d1f9a3-2aaf-4fad-9a1f-77fa91c8c798
...
ARTIST_ID2=9175a76f-7c4d-4a3e-be57-65856c6bb76i
...
cartist:
	echo curl --location --request POST 'http://$(IGW)/api/v1/artist/' --header 'Content-Type: application/json' --data-raw '$(BODY_ARTIST)' > $(LOG_DIR)/cartist.out
	$(CURL) --location --request POST 'http://$(IGW)/api/v1/artist/' --header 'Content-Type: application/json' --data-raw '$(BODY_ARTIST)' | tee -a $(LOG_DIR)/cartist.out

# PUT is used for user or artist (update) to update a record
uuser:
...
uartist:
	echo curl --location --request PUT 'http://$(IGW)/api/v1/artist/$(ARTIST_ID)' --header '$(TOKEN)' --header 'Content-Type: application/json' --data-raw '$(BODY_ARTIST)' > $(LOG_DIR)/uartist.out
	$(CURL) --location --request PUT 'http://$(IGW)/api/v1/artist/$(ARTIST_ID)' --header '$(TOKEN)' --header 'Content-Type: application/json' --data-raw '$(BODY_ARTIST)' | tee -a $(LOG_DIR)/uartist.out

# GET is used with music or artist to read a record
rmusic:
...
rartist:
	echo curl --location --request GET 'http://$(IGW)/api/v1/artist/$(ARTIST_ID)' --header '$(TOKEN)' > $(LOG_DIR)/rartist.out
	$(CURL) --location --request GET 'http://$(IGW)/api/v1/artist/$(ARTIST_ID)' --header '$(TOKEN)' | tee -a $(LOG_DIR)/rartist.out

# DELETE is used with user or music or artist to delete a record
duser:
...
dartist:
	echo curl --location --request DELETE 'http://$(IGW)/api/v1/artist/$(ARTIST_ID2)' --header '$(TOKEN)' > $(LOG_DIR)/dartist.out
	$(CURL) --location --request DELETE 'http://$(IGW)/api/v1/artist/$(ARTIST_ID2)' --header '$(TOKEN)' | tee -a $(LOG_DIR)/dartist.out
